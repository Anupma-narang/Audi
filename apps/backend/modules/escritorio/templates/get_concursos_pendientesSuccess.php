...
<?php echo format_number_choice('[0]Hay 0 concursos pendientes.|[1]Hay 1 concurso pendiente.|(1,+Inf]Hay %count% concursos pendientes.',
	...
	<?php foreach ($concursos->getResults() as $concurso):?>
		<li>
			<?php echo format_datetime($concurso->getCreatedAt(), "HH:mm d/MM/y", "es_ES")?> <?php //echo link_to($concurso->name,"concurso/show?id=".$concurso->id)?><?php echo link_to($concurso->name,"concursos_pendientes/show?id=".$concurso->id)?>
