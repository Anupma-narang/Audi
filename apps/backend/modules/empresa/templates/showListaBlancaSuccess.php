        ...
        <div class="sf_admin_list">
            <ul class="dragbox-content">
                ...
                <li>
                    ...
                    <?php if ($empresa->getMunicipioProvincia()): ?>
            ...
            <ul class="dragbox-content" style="float:left; width: 99%; margin-top: 0px;">
                ...
                <?php if ($empresa->getConcursoDestacado()->getId()) : ?>
                    ...
                    <li>
                        <strong>Concurso asociado: </strong>
                        <a href='<?php echo url_for('concurso_show', array('id' => $empresa->getConcursoDestacado()->getid())) ?>'><?php echo $empresa->getConcursoDestacado() ?></a>
                    </li>
                <?php endif ?>
            </ul>
            <div class="clean_clear_0_blanca"></div>
            ...
            <?php if ($pager->haveToPaginate()): ?>
                <div class="pagination">
                    ...
                    <?php echo link_to(image_tag('/images/first.png', array('title' => 'Primero')), 'empresa/show?page=' . $pager->getFirstPage() . '&pg=' . $sf_request->getParameter('pg') . '&id=' . $empresa->id) ?>
                    <?php echo link_to(image_tag('/images/previous.png', array('title' => 'Anterior')), 'empresa/show?page=' . $pager->getPreviousPage() . '&pg=' . $sf_request->getParameter('pg') . '&id=' . $empresa->id) ?>
                    <?php foreach ($pager->getLinks() as $page): ?>
                        <?php echo ($page == $pager->getPage()) ? $page : link_to($page, 'empresa/show?page=' . $page . '&pg=' . $sf_request->getParameter('pg', 1) . '&id=' . $empresa->id) ?>
                        <?php if ($page != $pager->getCurrentMaxLink()): ?>
                            -
                        <?php endif ?>
                    <?php endforeach ?>
                    <?php echo link_to(image_tag('/images/next.png', array('title' => 'Siguiente')), 'empresa/show?page=' . $pager->getNextPage() . '&pg=' . $sf_request->getParameter('pg') . '&id=' . $empresa->id) ?>
                    <?php echo link_to(image_tag('/images/last.png', array('title' => 'Último')), 'empresa/show?page=' . $pager->getLastPage() . '&pg=' . $sf_request->getParameter('pg') . '&id=' . $empresa->id) ?>
                </div>
            ...
            <?php if (count($pager_pending) > 0) : ?>
                ...
                <ul class="dragbox-content" style="float:left; width:98%; min-height: 0px;">
                    <li style="float:left; width:98%;"><strong><h3 style="margin: 0;">Auditorías pendientes</h3></strong></li>
                </ul>
            ...
            <?php if ($pager_pending->haveToPaginate()): ?>
                <div class="pagination">
                    ...
                    <?php echo link_to(image_tag('/images/first.png', array('title' => 'Primero')), 'empresa/show?pg=' . $pager_pending->getFirstPage() . '&page=' . $sf_request->getParameter('page') . '&id=' . $empresa->id) ?>
                    <?php echo link_to(image_tag('/images/previous.png', array('title' => 'Anterior')), 'empresa/show?pg=' . $pager_pending->getPreviousPage() . '&page=' . $sf_request->getParameter('page') . '&id=' . $empresa->id) ?>
                    <?php foreach ($pager_pending->getLinks() as $page): ?>
                        <?php echo ($page == $pager_pending->getPage()) ? $page : link_to($page, 'empresa/show?pg=' . $page . '&page=' . $sf_request->getParameter('page', 1) . '&id=' . $empresa->id) ?>
                        <?php if ($page != $pager_pending->getCurrentMaxLink()): ?>
                            -
                        <?php endif ?>
                    <?php endforeach ?>
                    <?php echo link_to(image_tag('/images/next.png', array('title' => 'Siguiente')), 'empresa/show?pg=' . $pager_pending->getNextPage() . '&page=' . $sf_request->getParameter('page') . '&id=' . $empresa->id) ?>
                    <?php echo link_to(image_tag('/images/last.png', array('title' => 'Último')), 'empresa/show?pg=' . $pager_pending->getLastPage() . '&page=' . $sf_request->getParameter('page') . '&id=' . $empresa->id) ?>
                </div>
...
<style type="text/css">
    .empresa_ul_audit{ float: left;margin: 0 0 0 25px;min-height: 0px; }
    .empresa_ul_comment_audit{ float: left;margin: 12px 0 0 25px;min-height: 0px; }
    .empresa_audit{ float: left;width: 100%; }
    .empresa_date{ float: left;width: 135px; }
    .empresa_user_name{ float: left;width: 200px;padding-right: 35px; }
    .empresa_pun_total{ float: left;width: 150px; }
    .empresa_comentario{ float: left;width: 125px; }
    .empresa_comment_aprobar{ float: left;width: 200px; }
    .empresa_aprobar{ float: left;width: 83px; }
    .pagination{ float: left;width: 98%;margin: 5px 0 10px 25px; }
    .pagination .result{ color: #006400;float: left;width: 85px;margin: -2px 0 0 0; }
    .empresa_number{ float: left;margin-top: -1px;width: 23px; }
    .empresa_kpi{ padding-left: 16px !important; }
